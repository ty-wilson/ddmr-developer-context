...
- `claimTenantMigration` = transient flag, written once when a CSA token's `tenant_id` claim differs from the stored tenant. The migration job (scans `claimTenantMigration_index`) remaps the tenant's DSS rows old→new, then sets `platformTenant`. **This job cannot run:** the `tenant_index` GSI it needs on the DSS table was removed in DDMR-1035 (2026-04-02) from all envs, and as of 2026-07 the job is not deployed (no live CronJob/ArgoCD Application in stage/prod).
