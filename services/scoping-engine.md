...
**Device-sync retry path is a self-feedback loop with no max-retry bound.** When `DeviceSyncHandler.processEvent` encounters a retryable error (e.g., `M2MTokenAcquisitionException(retryable=true)` from a failing HAL token fetch), the affected channel is added to `retriesNeeded` and `deviceService.syncDeviceWithDelay(tenant, device, SYNC_ERROR_DELAY)` is called (`DeviceSyncHandler.kt:121-123`, `SYNC_ERROR_DELAY = 30.seconds`). That produces a brand-new `DeviceSyncEvent` on `pdd/scoping-engine/device-sync` with a Pulsar `deliverAfter` delay (`PulsarService.sendDeviceSyncEvent`); the original message is acked normally. Consequence: a permanently failing device-channel — for example a tenant whose M2M client binding has been removed but whose `DEVICE-CHANNEL#<tenant>|<device>` row remains in DDB — will cycle forever at the steady-state retry rate per stuck channel. There is no consecutive-failure counter; the loop only stops when (a) the underlying failure resolves, (b) the DDB rows for the device are removed so `determineChannels` returns empty, or (c) the in-flight retried message ages out past Pulsar's max-redelivery setting. Diagnostic signature: high-volume `WARN com.jamf.platform.scoping.service.deployables.DeployableObjectSyncAdapter — "Exception during sync for ... M2M error, retrying"` for the same `device='...'` + `channel='...'` pair, often vastly exceeding what a 30 s scheduled retry would predict because each retry can spawn additional Pulsar redeliveries on top of the scheduled one.

**Pre-event idempotency vs per-adapter idempotency.** `DeviceSyncHandler` performs two distinct skip checks: the top-level "successfully synced after event trigger time" message (DEBUG) fires once per event when the device's stored sync state is newer than the event's `triggerTime`. The per-adapter "No sync needed on (device # channel) for {tag}" message (DEBUG) fires inside `filterChannelsForAdapter` and is emitted once per (channel, adapter) combination that has no work. Both can produce multiple DEBUG lines per event, which makes log-line counts a poor proxy for event counts when extrapolating throughput.
