...
**Retry path has no max-retry bound.** A retryable error in `DeviceSyncHandler.processEvent` (typically `M2MTokenAcquisitionException(retryable=true)`) calls `syncDeviceWithDelay(tenant, device, 30s)`, producing a fresh `DeviceSyncEvent` with a Pulsar `deliverAfter` delay (`DeviceSyncHandler.kt:121-123`). A device-channel with a permanent failure — e.g., orphan DDB state for a tenant no longer in HAL — loops forever until the DDB state is removed or Pulsar's max-redelivery cuts in.
