...
| `POST` | `/api/v1/strict/components/validate` | Stub endpoint — currently returns `200 { errors: [] }` without running any validation at all. Do not rely on this endpoint to catch declaration errors. |
...
**Lenient vs. strict path.** The `/components/*` (lenient) endpoints run schema and business-rule validation but only log issues — they never reject a request on validation grounds alone (translate logs at `debug`, validate logs at `warn`). The `/strict/components/*` endpoints are intended to enforce validation more strictly, but the strict validate endpoint is currently a stub that skips validation entirely and returns an empty error list. The strict translate does run validation internally via `createDeclarationsStrict`. Do not rely on the validate endpoint to surface errors to end users; check the service logs.
...
**Serialization.** Uses `kotlinx.serialization` (not Jackson) for request/response models. The codec is configured with `ignoreUnknownKeys = true` and `decodeEnumsCaseInsensitive = true`. `ChannelType` values (`SYSTEM`, `USER`) are case-insensitive. Note: `encodeDefaults` is not set (defaults to `false`).
